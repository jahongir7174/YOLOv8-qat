...
class ComputeLoss:
    ...
    def __call__(self, outputs, targets):
        loss = torch.zeros(2, device=self.device)  # cls, box

        shape = outputs[0].shape

        x_cat = torch.cat([i.view(shape[0], self.no, -1) for i in outputs], 2)
        pred_distri, pred_scores = x_cat.split((4, self.nc), 1)
        ...
        if box_targets.shape[0] == 0:
            gt = torch.zeros(shape[0], 0, 5, device=self.device)
        else:
            i = box_targets[:, 0]  # image index
            _, counts = i.unique(return_counts=True)
            counts = counts.to(dtype=torch.int32)
            gt = torch.zeros(shape[0], counts.max(), 5, device=self.device)
        ...
        pred_bboxes = self.box_decode(anchor_points, pred_distri)  # xyxy, (b, h*w, 4)
        ...
        if fg_mask.sum():
            ...
            loss[1] = self.box_loss(pred_bboxes,
                                    target_bboxes,
                                    target_scores,
